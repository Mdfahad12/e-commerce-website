...
function Cart() {
  ...
  const buyNow = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/api/auth/create-order', {
        method: 'POST',
        headers: {
        ...
        handler: async (response) => {
          ...
          if (response.razorpay_payment_id) {
            try {
              ...
              const backendResponse = await fetch('http://localhost:8000/api/auth/confirm-order', {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                },
                ...
                <div key={index} className="justify-between mb-6 rounded-lg border drop-shadow-xl bg-white p-6 sm:flex sm:justify-start" style={{ backgroundColor: mode === 'dark' ? 'rgb(32 33 34)' : '', color: mode === 'dark' ? 'white' : '' }}>
                  ...
                  <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                    <div className="mt-5 sm:mt-0">
                      <h2 className="text-lg font-bold text-gray-900" style={{ color: mode === 'dark' ? 'white' : '' }}>{name}</h2>
